...
public:
    ...
    [[nodiscard]] T get_return_value() const noexcept {
        return coroutine_.promise().get_return_value();
    }

    void wait() const noexcept {
        coroutine_.promise().get_atomic_flag().wait(false,
...
public:
    ...
    public:
        ...
        void await_suspend(std::coroutine_handle<sync_wait_task_promise<T>>
                               coroutine) const noexcept {
            std::atomic_flag& atomic_flag =
                coroutine.promise().get_atomic_flag();
    ...
    std::atomic_flag& get_atomic_flag() noexcept {
        return atomic_flag_;
    }
...
public:
    sync_wait_task<T> get_return_object() noexcept {
        return sync_wait_task<T>{
            std::coroutine_handle<sync_wait_task_promise<T>>::from_promise(
                *this)};
    }
...
sync_wait_all(std::vector<task<T>>& task_list) {
    if constexpr (std::is_same_v<T, void>) {
        ...
        std::transform(task_list.begin(), task_list.end(),
                       std::back_inserter(return_value_list),
