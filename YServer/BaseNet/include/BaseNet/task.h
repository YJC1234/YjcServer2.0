    ...
    ~task() noexcept {
        if (coroutine_) {
            if (coroutine_.done()) {
                ...
                coroutine_.promise().get_detached_flag().test_and_set(
...
public:
    ...
    public:
        ...
        [[nodiscard]] auto await_suspend(
            ...
            -> std::coroutine_handle<> {
            if (coroutine.promise().get_detached_flag().test(
                    std::memory_order_relaxed)) {
                coroutine.destroy();
            }
            return coroutine.promise().get_calling_coroutine().value_or(
                std::noop_coroutine());
        }
    };
    ...
    [[nodiscard]] auto get_calling_coroutine() noexcept
        -> std::optional<std::coroutine_handle<>>& {
        return calling_coroutine_;
    }

    [[nodiscard]] auto get_detached_flag() noexcept -> std::atomic_flag& {
...
public:
    ...
    auto return_value(U&& return_value) noexcept(
        std::is_nothrow_constructible_v<T, U&&>) -> void {
        return_value_.emplace(std::forward<U>(return_value));
    }

    [[nodiscard]] auto get_return_value() & noexcept -> T& {
