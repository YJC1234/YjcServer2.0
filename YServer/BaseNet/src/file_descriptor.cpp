...
std::strong_ordering
file_descriptor::operator<=>(const file_descriptor& other) const {
...
splice_awaiter::splice_awaiter(const int    raw_file_descriptor_in,
                               const int    raw_file_descriptor_out,
                               const size_t length)
    : raw_file_descriptor_in_{raw_file_descriptor_in},
      raw_file_descriptor_out_{raw_file_descriptor_out},
      length_{length} {}

bool splice_awaiter::await_ready() const {
    return false;
}
...
ssize_t splice_awaiter::await_resume() const {
    ...
    while (bytes_sent < length) {
        {
            ssize_t result = co_await splice_awaiter(
                file_descriptor_in.get_raw_file_descriptor(),
                write_pipe.get_raw_file_descriptor(), length);
            if (result < 0) {
                co_return -1;
            }
        }
        {
            ssize_t result = co_await splice_awaiter(
                ...
                file_descriptor_out.get_raw_file_descriptor(), length);
            if (result < 0) {
                co_return -1;
            }
            bytes_sent += result;
