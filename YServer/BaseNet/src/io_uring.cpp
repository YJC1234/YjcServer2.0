...
io_uring::cqe_iterator::cqe_iterator(const ::io_uring*  io_uring,
...
auto io_uring::cqe_iterator::operator*() const noexcept -> io_uring_cqe* {
    return &io_uring_->cq.cqes[io_uring_cqe_index(io_uring_, head_,
                                                  (io_uring_)->cq.ring_mask)];
}

auto io_uring::cqe_iterator::operator!=(
    const cqe_iterator& other) const noexcept -> bool {
    return head_ != other.head_;
}

auto io_uring::begin() -> cqe_iterator {
    return cqe_iterator{&io_uring_, *io_uring_.cq.khead};
}

auto io_uring::end() -> cqe_iterator {
    return cqe_iterator{&io_uring_,
                        io_uring_smp_load_acquire(io_uring_.cq.ktail)};
}

auto io_uring::cqe_seen(io_uring_cqe* const cqe) -> void {
...
auto io_uring::submit_and_wait(const int wait_nr) -> int {
    ...
    if (result < 0) {
        spdlog::get("system_logger")
            ->error("failed to invoke 'io_uring_submit_and_wait'");
        throw std::runtime_error("failed to invoke 'io_uring_submit_and_wait'");
    }
    return result;
}

auto io_uring::submit_multishot_accept_request(sqe_data*  sqe_data,
                                               const int  raw_file_descriptor,
                                               sockaddr*  client_addr,
                                               socklen_t* client_len) -> void {
    io_uring_sqe* sqe = io_uring_get_sqe(&io_uring_);
    io_uring_prep_multishot_accept(sqe, raw_file_descriptor, client_addr,
                                   client_len, 0);
    io_uring_sqe_set_data(sqe, sqe_data);
}

auto io_uring::submit_recv_request(sqe_data*    sqe_data,
                                   const int    raw_file_descriptor,
...
auto io_uring::submit_send_request(sqe_data*              sqe_data,
                                   const int              raw_file_descriptor,
...
auto io_uring::submit_splice_request(sqe_data* sqe_data,
                                     int       raw_file_descriptor_in,
                                     int raw_file_descriptor_out, size_t length)
    -> void {
    io_uring_sqe* sqe = io_uring_get_sqe(&io_uring_);
    io_uring_prep_splice(sqe, raw_file_descriptor_in, -1,
                         raw_file_descriptor_out, -1, length, 0);
    io_uring_sqe_set_data(sqe, sqe_data);
}
...
auto io_uring::setup_buffer_ring(io_uring_buf_ring*           buffer_ring,
    ...
    if (result != 0) {
        spdlog::get("system_logger")
            ->error("failed to invoke 'io_uring_register_buf_ring'");
        throw std::runtime_error(
            "failed to invoke 'io_uring_register_buf_ring'");
    }
    io_uring_buf_ring_init(buffer_ring);

    const unsigned int mask = io_uring_buf_ring_mask(buffer_ring_size);
    for (unsigned int buffer_id = 0; buffer_id < buffer_ring_size;
         ++buffer_id) {
        io_uring_buf_ring_add(buffer_ring, buffer_list[buffer_id].data(),
                              buffer_list[buffer_id].size(), buffer_id, mask,
                              buffer_id);
    }
    io_uring_buf_ring_advance(buffer_ring, buffer_ring_size);
};

auto io_uring::add_buffer(io_uring_buf_ring* buffer_ring,
                          std::span<char> buffer, const unsigned int buffer_id,
                          const unsigned int buffer_ring_size) -> void {
    const unsigned int mask = io_uring_buf_ring_mask(buffer_ring_size);
